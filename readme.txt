...
* Fixed: Removed stale `simple-reservation-system.php` entry point that caused WordPress to detect the wrong plugin name and version
...
* Added: Access indicator in the resources admin table — lock icon shows whether a resource is restricted and how many users have access
* Fixed: Frontend reservation form and calendar filter now only show resources the current user has access to
* Fixed: Auto-update check from GitHub releases via `WPPluginfactory_Updater`

= 2.5.8 =
* Test: pipeline smoke test — readme.md conversion + changelog-based release notes
